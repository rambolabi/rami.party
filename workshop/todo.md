...
| # | Area | Open items |
|---|---|---|
| [0](#0-site-wide--critical) | Site-wide / critical | 12 |
...
| [4](#4-dd-forge-workshopdd-forge) | D&D Forge | 22 |
...
| [9](#9-laser-works-workshoplaser) | Laser Works | 1 (🐞 15 broken images) |
| [10](#10-the-people-library-workshoppersonality-library--15-book-workshops) | The People Library (15 workshops) | 3 |
| [11](#11-tapfate--random-first-player-wastelandrandom-first-player) | TapFate / Random First Player | 6 |
| [12](#12-ghosttooth-copy-wastelandbluetooth) | GhostTooth copy (wasteland) | 2 |
...
### 🔗 0.1 — D&D Forge is completely orphaned **(highest impact)**
`workshop/D&D Forge/` contains a fully shipped v1.6 app (muster page, 33 races, 141 spells,
DM Charter, play mode) but **nothing on the site links to it**. `RAMI_PLANNED` in
[projects.js](../projects.js) still says *"D&D — folder claimed, spell not yet cast."*

**Fix**
1. Remove `{ name: 'D&D', … }` from `window.RAMI_PLANNED`.
2. Append to `window.RAMI_WORKSHOP`:
   ```js
   {
       title: 'D&D Forge',
       href: './D%26D%20Forge/',   // URL-encode the & and space
       glyph: '🐉',
       tagline: 'Forge a hero',
       description: 'A full D&D character builder across six editions — wizard-driven sheet, dice roller, DM Charter and party muster.',
       search: 'Dungeons and Dragons character creator and builder for 5E 2024, 5E 2014, 3.5, 4E, AD&D 2E and 1E. Roll ability scores, pick race class background feats spells and equipment, dice roller, printable character sheet, DM charter and party muster.',
       tags: ['app', 'ttrpg'],
       aura: 'gold',
       status: 'live',
   }
   ```
3. **Better:** rename the folder `D&D Forge` → `dnd-forge`. Spaces and `&` in a path are a
   permanent source of encoding bugs on GitHub Pages. Update the two internal links in
   `index.html`/`muster.html` after renaming.

### 🔗 0.2 — The People Library + all 15 book workshops are orphaned
`workshop/personality-library/index.html` is a working hub linking 15 delivered workshops
(DISC, Big Five, MBTI, Enneagram, Attachment, Love Languages, Temperaments, Conflict Styles,
EQ, Liars, Vampires, Bad Bosses, Psychopaths, Narcissists, Setbacks). None appear in
`projects.js`, so they are unreachable from the hub and the on-site search.

**Fix**
1. Add **one** catalogue entry for the library (not 15) to `RAMI_WORKSHOP`:
   ```js
   {
       ...
       description: 'A shelf of interactive workshops on personality and behaviour — DISC colours, Big Five, MBTI, the Enneagram, attachment styles, manipulation, narcissism and more.',
       search: 'Interactive personality and psychology workshops with quizzes and self assessments: DISC Surrounded by Idiots colour profile, Big Five OCEAN, MBTI 16 types, Enneagram, attachment styles, five love languages, four temperaments, Thomas-Kilmann conflict styles, emotional intelligence EQ, Surrounded by Liars deception, energy vampires, bad bosses leadership, psychopaths manipulation, narcissists, resilience and setbacks.',
       tags: ['workshop', 'psychology'],
       aura: 'pink',
       status: 'live',
   }
   ```
2. Remove the stale `{ name: 'DISC Profile', … }` entry from `RAMI_PLANNED` — it shipped as v2.

### 🐞 0.3 — `sitemap.xml` advertises pages that do not exist
[sitemap.xml](../sitemap.xml) lists `/test`, `/demo`, `/coffee`, `/404` and `/wasteland` with a
2026-01-11 `lastmod`. Four of six URLs 404. Real sections (`/workshop/`, `/gallery/lore/`,
`/gallery/prankscreens/`, `/wasteland/neko/`) are missing entirely.

**Fix** — rewrite the sitemap with only real URLs:
`/`, `/workshop/`, `/gallery/lore/`, `/gallery/prankscreens/`, `/wasteland/`,
`/wasteland/neko/`, `/wasteland/old-rami.party/`, `/wasteland/notes/`, plus each live
workshop project. Refresh `lastmod`. Ideally generate it from `projects.js` with a small script.

### 0.4 — The whole site is `noindex`
19 pages carry `<meta name="robots" content="noindex">` while `robots.txt` says
*"Google, you're cool. Come on in."* and links a sitemap — contradictory signals.

**Fix** — decide, then be consistent:
- **Publish:** strip `noindex` from `index.html`, `404.html` (keep noindex here), `workshop/index.html`,
  `wasteland/index.html`, `gallery/*/index.html` and the project pages; add `<link rel="canonical">`
  to each; fix the sitemap first (0.3).
- **Stay private:** leave `noindex`, but remove the `Sitemap:` line and the "come on in"
  allowances from `robots.txt` so the signals agree.

### 🐞 0.5 — `workshop/tools/cve/` loads a stylesheet that doesn't exist here
[tools/cve/index.html](tools/cve/index.html#L7) has
`<link rel="stylesheet" href="/style.css">` — an absolute root path left over from lebon.info.
On rami.party this resolves to the **hub-only** stylesheet, so the CVE terminal renders unstyled/wrong.

**Fix** — point it at the local tools theme: `href="../style.css"` (or copy the needed rules
into `cve.css`). Then remove the `lebon.info` title/prompt strings (see 0.6).

### 0.6 — Residual lebon.info branding
`lebon.info` still appears in:
[tools/index.html](tools/index.html) (lines 9, 10, 32, 144, 2316, 2328),
[tools/script.js](tools/script.js#L120),
[tools/style.css](tools/style.css#L366),
[tools/cve/index.html](tools/cve/index.html) (6, 13),
[news/theme.css](news/theme.css#L366),
[webcheck/about.html](webcheck/about.html#L160),
[webcheck/style.css](webcheck/style.css#L2),
[../wasteland/bluetooth/style.css](../wasteland/bluetooth/style.css#L3).

**Fix** — search-and-replace `lebon.info` → `rami.party` in titles, prompts and comments.
**Do not** touch `workshop/bbq/` (see 0.7) — that address is a real recipient.

### 0.7 — BBQ Mailer has no working backend
`workshop/bbq/` is listed as live in `RAMI_WORKSHOP`, but
[bbq/script.js](bbq/script.js#L1590) pushes `BBQ@lebon.info` and there is no
server-side handler — rami.party is static (GitHub Pages), so the form cannot actually send.

**Fix** — pick one:
- Point the form at a static-friendly backend (Formspree / Web3Forms / a Cloudflare Worker), **or**
- Keep it as a `mailto:` composer only, **or**
- Flip the catalogue entry to `external: true` and `href: 'https://lebon.info/bbq/'` until a
  backend exists.

### 0.8 — `followUp.md` is stale
It still describes the `workshop/projects/` migration as "this round" and lists items that are
now done (bluetooth/news back-links) or superseded.

**Fix** — replace its "Still open" section with a pointer to this file and keep `followUp.md`
for decisions only.

### 0.9 — Duplicate GhostTooth
`wasteland/bluetooth/` duplicates the standalone `ghosttooth.labidi.eu` project (linked from the
hub as a Gallery realm).

**Fix** — decide: keep the wasteland copy as a preserved relic (add a banner "superseded by
ghosttooth.labidi.eu"), or delete the folder and drop its `TODO.md`.

### 0.10 — Per-project completeness pass
Every `RAMI_WORKSHOP` entry is `status: 'live'`. Several (`dashboard_v1`, `rami`, `laser`,
`to-check`) are stubs or broken.

**Fix** — set `status: 'soon'` on anything not presentable so it renders as a locked teaser.

### 0.11 — Grow `RAMI_PLANNED`
Only *Screen Fill* remains genuinely planned after 0.1/0.2 remove the other two.

**Fix** — add real future ideas as `{ name, note }` objects.

### 0.12 — Add a `README.md` per project folder
Only 6 of ~40 project folders have one. New contributors (and future you) can't tell what
`dashboard_v1`, `rami`, `to-check`, `trace-results` or `clear` are.

**Fix** — drop a 5-line README in each: what it is, how to run it, current status.
...
> built yet, yet it is listed as `status: 'live'` in `projects.js` with a description promising
> Web Serial control. **🐞 That description is currently false advertising** — set it to
> `status: 'soon'` until §2.3 lands, or reword the copy.
...
## 4. D&D Forge — `workshop/D&D Forge/`

v1.6 shipped. See also [0.1](#-01--dd-forge-is-completely-orphaned-highest-impact).

### 🐞 Bugs / risks
- **🐞 Folder name contains `&` and a space.** Guaranteed URL-encoding pain on GitHub Pages.
  *Fix:* rename to `dnd-forge/` and update internal links.
...
- [ ] Local `assets/` — currently relies on Google Fonts + emoji; bundle for offline use.
...
> 🔗 **Registry drift:** the `projects.js` entry still says *"powered by showdown"*, but the
> CDN dependency was removed and replaced with a hand-written parser.
> **Fix:** update the `description`/`search` text to say "fully offline, dependency-free".
...
- [x] ~~Add it to the workshop catalogue~~ — **already done** (listed as *Disk Filler*).
      *Fix: tick this line in [fill/todo.md](fill/todo.md#L61).*
...
### 🐞 **15 broken images — the page is visibly broken**
[laser/index.html](laser/index.html) references `./media/*.jpg` fifteen times
(hero, 3 service cards, 8 gallery tiles, 3 testimonial avatars) but **there is no `media/`
folder** — the whole folder is just `index.html`, `script.js`, `style.css`, `translations.js`.

**Fix** — pick one:
1. Add a `media/` folder with the 15 named JPEGs, **or**
2. Replace `<img>` with CSS gradient/SVG placeholders and delete the `alt`-only tiles, **or**
3. Set the catalogue entry to `status: 'soon'` so it renders as a locked teaser instead of a
   broken page (fastest, and honest — the description already says "still finding its words").
...
- [ ] **Surface the library on the hub** — see [0.2](#-02--the-people-library--all-15-book-workshops-are-orphaned).
- [ ] **Chart.js is loaded from a CDN** (DISC workshop) while every other rami.party project is
      ...
      hand-rolled canvas renderer to match the "no third-party requests" standard.
...
- [ ] **Optional phase 2 of item 4** — per-device RSSI history (capped ~50-sample ring buffer)
      rendered as an inline sparkline.
- [ ] **Resolve the duplication** with `ghosttooth.labidi.eu` — see [0.9](#09--duplicate-ghosttooth).
      Also strip the `lebon.info` reference in `style.css` line 3.
...
| `workshop/bbq/` | BBQ Mailer | **backend missing** — see [0.7](#07--bbq-mailer-has-no-working-backend) |
...
| `workshop/dashboard_v1/` | Window View | **stub** — likely `status: 'soon'` |
| `workshop/DNS-sinkhole/` | DNS Sinkhole | live, undocumented |
| `workshop/Engraving/` | Engraving Gallery | live, undocumented |
| `workshop/news/` | IT News Scroll | live; `theme.css` still says lebon.info |
| `workshop/rami/` | Tinkerer's Desk | **scratch page** — likely `status: 'soon'` |
...
| `workshop/tools/` | IT Tools Workbench | **🐞 CVE sub-page broken** — see [0.5](#-05--workshoptoolscve-loads-a-stylesheet-that-doesnt-exist-here) |
| `workshop/trace-results/` | Volume Seer | live, undocumented |
| `workshop/webcheck/` | Webcheck | live; lebon.info branding |
| `wasteland/adhd/` | ADHD experiment | empty husk (intentional) |
| `wasteland/house/` | — | **not in any registry** — surface or delete |
...
1. **0.1 + 0.2** — surface D&D Forge and The People Library (16 finished apps currently invisible).
2. **9** — fix or hide the broken Laser Works page.
3. **0.5 + 0.6** — fix the CVE stylesheet, then sweep lebon.info branding.
4. **0.3 + 0.4** — decide on indexing, then rewrite `sitemap.xml`.
5. **0.10 + 2** — set honest `status` flags (especially laser-forge-v2).
6. **0.7** — decide the BBQ backend.
7. **3 (CSG booleans)** and **2.1 (multi-select)** — the two biggest single feature unlocks.
8. **0.12 + 13** — README + audit pass across the undocumented folders.
