/* ==========================================================================
   Loregate — Dungeon Master's screen controller (v3)
   Locations, painted terrain, placeable items, weather, fog with adjustable
   see-through, brush sizes, a blackout, player dice, reorderable panels and a
   live-mirrored battlemap.
   ========================================================================== */
(function () {
    'use strict';
    ...
    let mode = 'select';      // select | paint | erase | brush | hide | ping | measure | place | wxerase
    ...
    function cellFromPointer(evt) {
        ...
        const x = clamp((evt.clientX - rect.left) / rect.width, 0, 0.9999);
        const y = clamp((evt.clientY - rect.top) / rect.height, 0, 0.9999);
    ...
    function autoLight(L, settings) {
        ...
        const sources = L.tokens.filter((t) => t.kind === 'player')
            .concat(L.items.filter((i) => i.type === 'torch' || i.type === 'campfire' || i.type === 'lantern'));
        sources.forEach((s) => {
            for (let dr = -rad; dr <= rad; dr++) for (let dc = -rad; dc <= rad; dc++)
                if (Math.hypot(dc, dr) <= rad) {
                    const c = s.c + dc, r = s.r + dr;
                    if (c >= 0 && r >= 0 && c < L.map.cols && r < L.map.rows) set.add(c + ',' + r);
                }
        });
        L.fog = Array.from(set);
    }

    /* ---- Weather mask for the ambient layer ---- */
    ...
    function renderMap() {
        ...
        if (terrainLayer.childElementCount !== cols * rows) {
            ...
            for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
                ...
                cell.className = 'terrain-cell'; cell.dataset.key = c + ',' + r;
        ...
        if (set.showCoords && coordLayer.dataset.size !== cols + 'x' + rows) {
            coordLayer.dataset.size = cols + 'x' + rows;
            let html = '';
        ...
        if (fogLayer.childElementCount !== cols * rows) {
            ...
            for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) {
                const cell = document.createElement('div');
                cell.className = 'fog-cell'; cell.dataset.key = c + ',' + r;
        ...
        L.tokens.forEach((t) => {
            ...
            const isNew = !seenTokens.has(t.id);
            el.className = 'token draggable kind-' + t.kind + (t.hidden ? ' is-hidden' : '') + (dead ? ' is-dead' : '') +
                (st.turn.activeId === t.id ? ' is-active' : '') + (isNew && anim ? ' spawn-in' : '');
    ...
    mapFrame.addEventListener('pointerdown', (e) => {
        if (mode === 'paint' || mode === 'erase') { e.preventDefault(); painting = true; lastPaintKey = null; mapFrame.setPointerCapture(e.pointerId); paintTerrain(e); }
        ...
        else if (mode === 'wxerase') { e.preventDefault(); painting = true; lastPaintKey = null; mapFrame.setPointerCapture(e.pointerId); paintWeatherErase(e); }
            ...
            const piece = e.target.closest('.draggable');
            if (!piece) return;
            e.preventDefault();
            drag = { el: piece, id: piece.dataset.id, kind: piece.dataset.kind, moved: false };
    ...
    mapFrame.addEventListener('pointermove', (e) => {
        if (drag) {
            const rect = mapFrame.getBoundingClientRect();
            drag.el.style.left = clamp((e.clientX - rect.left) / rect.width * 100, 0, 100) + '%';
            drag.el.style.top = clamp((e.clientY - rect.top) / rect.height * 100, 0, 100) + '%';
            drag.moved = true;
        } else if (painting && (mode === 'paint' || mode === 'erase')) paintTerrain(e);
        else if (painting && (mode === 'brush' || mode === 'hide')) paintFog(e);
        else if (painting && mode === 'wxerase') paintWeatherErase(e);
        else if (measuring) drawMeasure(e);
    });
    window.addEventListener('pointerup', (e) => {
        if (drag) {
            const { id, kind, moved } = drag;
            drag.el.classList.remove('dragging'); const wasDrag = moved; drag = null;
            if (wasDrag) {
                const { c, r } = cellFromPointer(e);
                S.update((st) => {
                    const L = S.activeLocation(st);
                    const arr = kind === 'token' ? L.tokens : L.items;
                    const p = arr.find((x) => x.id === id);
                    if (p) { p.c = c; p.r = r; }
                    if (kind === 'token') autoLight(L, st.settings);
                });
            }
        }
    ...
    function paintTerrain(evt) {
        const { c, r } = cellFromPointer(evt); const key = c + ',' + r;
        if (key === lastPaintKey) return; lastPaintKey = key;
        const terr = $('#terrainSelect').value;
        S.update((st) => { const L = S.activeLocation(st); if (mode === 'erase') delete L.terrain[key]; else L.terrain[key] = terr; });
    }
    function paintFog(evt) {
        const { c, r } = cellFromPointer(evt);
        const size = Number($('#brushSize').value) || 1;
        const half = Math.floor(size / 2);
        const reveal = evt.altKey ? (mode !== 'brush') : (mode === 'brush');
        S.update((st) => {
            const L = S.activeLocation(st);
            const fset = new Set(L.fog);
            for (let dr = -half; dr <= half; dr++) for (let dc = -half; dc <= half; dc++) {
                const cc = c + dc, rr = r + dr;
                if (cc < 0 || rr < 0 || cc >= L.map.cols || rr >= L.map.rows) continue;
                const key = cc + ',' + rr;
                if (reveal) fset.add(key); else fset.delete(key);
            }
            L.fog = Array.from(fset);
        });
    }
    function paintWeatherErase(evt) {
        const { c, r } = cellFromPointer(evt); const key = c + ',' + r;
        if (key === lastPaintKey) return; lastPaintKey = key;
        S.update((st) => {
            const L = S.activeLocation(st);
            const set = new Set(L.weatherExclude || []);
            if (evt.altKey) set.delete(key); else set.add(key);
            L.weatherExclude = Array.from(set);
        });
    }
    function drawMeasure(evt) {
        const L = S.loc();
        const rect = mapFrame.getBoundingClientRect();
        const x1 = evt.clientX - rect.left, y1 = evt.clientY - rect.top;
        const dx = x1 - measuring.x0, dy = y1 - measuring.y0;
        const len = Math.hypot(dx, dy);
        const ang = Math.atan2(dy, dx) * 180 / Math.PI;
        const cellPx = rect.width / L.map.cols;
        const squares = Math.round(len / cellPx);
        clearMeasure();
        const line = document.createElement('div');
        line.className = 'measure-line';
        line.style.left = measuring.x0 + 'px'; line.style.top = measuring.y0 + 'px';
        line.style.width = len + 'px'; line.style.transform = `rotate(${ang}deg)`;
        const label = document.createElement('div');
        label.className = 'measure-label';
        label.style.left = (measuring.x0 + dx / 2) + 'px'; label.style.top = (measuring.y0 + dy / 2) + 'px';
        label.textContent = `${squares} sq · ${squares * 5} ft · ${(squares * 1.5).toFixed(1)} m`;
        fxLayer.append(line, label);
    }
    function clearMeasure() { fxLayer.querySelectorAll('.measure-line,.measure-label').forEach((n) => n.remove()); }

    const MODE_BTNS = [['paintBtn', 'paint'], ['eraseBtn', 'erase'], ['brushBtn', 'brush'], ['hideBrushBtn', 'hide'], ['pingBtn', 'ping'], ['measureBtn', 'measure'], ['wxEraseBtn', 'wxerase']];
    function setMode(next) {
        mode = (mode === next && next !== 'select') ? 'select' : next;
        if (mode !== 'place') pendingItem = null;
        MODE_BTNS.forEach(([id, m]) => $('#' + id).classList.toggle('active', mode === m));
    ...
    $('#locDelete').addEventListener('click', () => {
        const st = S.get();
        if (st.locations.length <= 1) { toast('Keep at least one location'); return; }
        if (st.settings.confirmDelete && !confirm('Delete “' + S.loc().name + '”?')) return;
        S.update((s) => { s.locations.splice(s.activeLoc, 1); s.activeLoc = Math.max(0, s.activeLoc - 1); });
    });
    $('#locExport').addEventListener('click', () => {
        const blob = new Blob([JSON.stringify(S.loc(), null, 2)], { type: 'application/json' });
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
        a.download = 'loregate-location-' + (S.loc().name || 'map').replace(/\s+/g, '-').toLowerCase() + '.json';
        a.click(); URL.revokeObjectURL(a.href);
    });
    $('#locImport').addEventListener('click', () => $('#locImportFile').click());
    $('#locImportFile').addEventListener('change', (e) => {
        const file = e.target.files[0]; if (!file) return;
        const reader = new FileReader();
        reader.onload = () => {
            try {
                const loc = JSON.parse(reader.result);
                if (!loc || !loc.map) throw 0;
                loc.id = 'loc-' + Math.random().toString(36).slice(2, 9);
                S.update((st) => { st.locations.push(loc); st.activeLoc = st.locations.length - 1; });
                toast('Location imported');
            } catch { toast('That file is not a location'); }
        };
        reader.readAsText(file); e.target.value = '';
    });

    /* ======================================================================
       TERRAIN / FOG / WEATHER TOOLBARS
       ====================================================================== */
    ...
    $('#fillBtn').addEventListener('click', () => {
        const terr = $('#terrainSelect').value;
        S.update((st) => { S.activeLocation(st).map.bg = terr; });
        toast('Base terrain set to ' + (TERRAIN[terr] ? TERRAIN[terr].label : terr));
    });
    ...
    $('#revealAll').addEventListener('click', () => S.update((st) => {
        const L = S.activeLocation(st), all = [];
        for (let r = 0; r < L.map.rows; r++) for (let c = 0; c < L.map.cols; c++) all.push(c + ',' + r);
        L.fog = all;
    }));
    ...
    $$('.wx').forEach((b) => b.addEventListener('click', () => {
        const wx = b.dataset.wx;
        S.update((st) => { const L = S.activeLocation(st); L.weather = (L.weather === wx) ? 'none' : wx; });
    }));
    ...
    (function buildPalette() {
        ...
        Object.keys(ITEMS).forEach((type) => {
            ...
            b.className = 'pal-item'; b.type = 'button'; b.dataset.type = type;
            b.title = ITEMS[type].label; b.textContent = ITEMS[type].icon;
            b.addEventListener('click', () => {
                pendingItem = type; mode = 'place';
                $$('#palette .pal-item').forEach((p) => p.classList.toggle('active', p === b));
                MODE_BTNS.forEach(([id]) => $('#' + id).classList.remove('active'));
                mapFrame.style.cursor = 'crosshair';
                toast('Click the map to place a ' + ITEMS[type].label);
            });
            pal.appendChild(b);
        });
    })();
    function renderItems() {
        const L = S.loc();
        const wrap = $('#itemList');
        $('#itemCount').textContent = L.items.length;
        wrap.innerHTML = '';
        L.items.forEach((it) => {
            const row = document.createElement('div');
            row.className = 'roster-item'; row.style.borderLeftColor = 'var(--arcane)';
            row.innerHTML = `
                <div class="swatch" style="background:transparent;border-color:var(--line);font-size:1rem">${it.icon}</div>
                <div class="roster-main">
                    <input class="j-name" value="${escapeHtml(it.name)}" style="background:transparent;border:0;padding:0;font-weight:600;font-size:.86rem;color:var(--text)">
                    <div class="roster-meta"><span>${it.type}</span><span>cell ${it.c},${it.r}</span></div>
                </div>
                <div class="icon-actions">
                    <button class="mini ${it.hidden ? '' : 'on'} j-vis" title="Show / hide from players">${it.hidden ? '🚫' : '👁️'}</button>
                    <button class="mini j-big" title="Cycle size">⤢</button>
                    <button class="mini danger j-del" title="Remove">🗑️</button>
                </div>`;
    ...
    $('#addTokenForm').addEventListener('submit', (e) => {
        ...
        const kind = $('#ntKind').value; const hp = Number($('#ntHp').value) || 10;
        const colors = { player: '#38bdf8', enemy: '#e5484d', npc: '#e8b24c' };
        S.update((st) => {
            const L = S.activeLocation(st);
            L.tokens.push(S.makeToken({
                name, kind, color: colors[kind], hp, maxHp: hp,
                c: 1 + (L.tokens.length % Math.max(1, L.map.cols - 2)),
                r: 1 + (Math.floor(L.tokens.length / 4) % Math.max(1, L.map.rows - 2)),
                initiative: kind === 'player' ? 10 : Math.ceil(Math.random() * 20),
            }));
            autoLight(L, st.settings);
        });
        e.target.reset(); $('#ntKind').value = kind; $('#ntHp').value = 10; $('#ntName').focus();
    });
    function renderRoster() {
        const st = S.get(); const L = S.loc();
        const wrap = $('#roster');
        $('#rosterCount').textContent = L.tokens.length;
        $('#rosterEmpty').style.display = L.tokens.length ? 'none' : 'block';
        wrap.innerHTML = '';
        L.tokens.forEach((t) => {
            const item = document.createElement('div');
            item.className = 'roster-item' + (st.turn.activeId === t.id ? ' active' : '');
            item.dataset.id = t.id; item.style.borderLeftColor = t.color;
            const ratio = t.maxHp > 0 ? clamp(t.hp / t.maxHp, 0, 1) : 0;
            item.innerHTML = `
                <div class="swatch" style="background:${t.color}">${(t.name || '?').slice(0, 2).toUpperCase()}</div>
                <div class="roster-main">
                    <div class="roster-name">${escapeHtml(t.name)} <span class="badge">${t.kind}</span></div>
                    <div class="roster-meta">
                        <span>AC ${t.ac}</span>
                        <span>Init <input class="hp-input j-init" type="number" value="${t.initiative}" style="width:40px"></span>
                    </div>
                    <div class="hp-bar"><span style="width:${ratio * 100}%;background:${hpColor(ratio)}"></span></div>
                    <div class="hp-controls" style="margin-top:.35rem">
                        <button class="hp-step j-dmg" title="Damage 1">−</button>
                        <input class="hp-input j-hp" type="number" value="${t.hp}">
                        <span style="color:var(--text-faint);font-size:.75rem">/ ${t.maxHp}</span>
                        <button class="hp-step j-heal" title="Heal 1">＋</button>
                    </div>
                </div>
                <div class="icon-actions">
                    <button class="mini ${t.hidden ? '' : 'on'} j-vis" title="Show / hide on player screen">${t.hidden ? '🚫' : '👁️'}</button>
                    <button class="mini ${t.showHp ? 'on' : ''} j-hp-toggle" title="Show HP to players (map + sidebar)">❤️</button>
                    <button class="mini j-big" title="Cycle size">⤢</button>
                    <button class="mini j-center" title="Center on map">🎯</button>
                    <button class="mini danger j-del" title="Remove">🗑️</button>
                </div>
                <div class="conditions">
                    ${CONDITIONS.map((c) => `<span class="cond ${t.conditions.includes(c) ? '' : 'off'}" data-cond="${c}">${c}</span>`).join('')}
                </div>`;
            ...
            item.querySelector('.j-del').addEventListener('click', () => {
                if (st.settings.confirmDelete && !confirm(`Remove ${t.name}?`)) return;
                S.update((s) => { const L2 = S.activeLocation(s); L2.tokens = L2.tokens.filter((x) => x.id !== t.id); if (s.turn.activeId === t.id) s.turn.activeId = null; });
            });
            item.querySelectorAll('.cond').forEach((chip) => chip.addEventListener('click', () => upd((tk) => {
                const cond = chip.dataset.cond;
                tk.conditions = tk.conditions.includes(cond) ? tk.conditions.filter((x) => x !== cond) : [...tk.conditions, cond];
            })));
    ...
    function renderInitiative() {
        const st = S.get(); const L = S.loc();
        const order = [...L.tokens].sort((a, b) => b.initiative - a.initiative);
        $('#roundPill').textContent = 'Round ' + st.turn.round;
        $('#initEmpty').style.display = order.length ? 'none' : 'block';
        $('#initList').innerHTML = order.map((t) => `
            <div class="init-row ${st.turn.activeId === t.id ? 'active' : ''}">
                <span class="init-num">${t.initiative}</span>
                <span class="swatch" style="width:20px;height:20px;background:${t.color}"></span>
                <span class="init-name">${escapeHtml(t.name)}</span>
                <span class="roster-meta">${t.maxHp > 0 ? t.hp + '/' + t.maxHp : ''}</span>
            </div>`).join('');
    }
    ...
    function advanceTurn(dir) {
        S.update((st) => {
            const order = [...S.activeLocation(st).tokens].sort((a, b) => b.initiative - a.initiative);
            if (!order.length) return;
            let idx = order.findIndex((t) => t.id === st.turn.activeId);
            if (idx === -1) idx = dir > 0 ? -1 : order.length;
            idx += dir;
            if (idx >= order.length) { idx = 0; st.turn.round += 1; }
            if (idx < 0) { idx = order.length - 1; st.turn.round = Math.max(1, st.turn.round - 1); }
            st.turn.activeId = order[idx].id;
        });
    }
    ...
    /* ======================================================================
       BESTIARY
       ====================================================================== */
    $('#addBeastForm').addEventListener('submit', (e) => {
        e.preventDefault();
        const name = $('#beName').value.trim(); if (!name) return;
        S.update((st) => st.bestiary.push({
            id: 'be-' + Math.random().toString(36).slice(2, 8),
            name, cr: $('#beCr').value.trim(), ac: Number($('#beAc').value) || 10,
            hp: Number($('#beHp').value) || 8, maxHp: Number($('#beHp').value) || 8, notes: $('#beNotes').value.trim(),
        }));
        e.target.reset(); $('#beName').focus();
    });
    function renderBestiary() {
        const st = S.get();
        $('#bestiaryCount').textContent = st.bestiary.length;
        $('#bestiaryEmpty').style.display = st.bestiary.length ? 'none' : 'block';
        const wrap = $('#bestiary'); wrap.innerHTML = '';
        st.bestiary.forEach((b) => {
            const item = document.createElement('div');
            item.className = 'roster-item'; item.style.borderLeftColor = 'var(--arcane)';
            item.innerHTML = `
                <div class="swatch" style="background:var(--arcane)">${(b.name || '?').slice(0, 2).toUpperCase()}</div>
                <div class="roster-main">
                    <div class="roster-name">${escapeHtml(b.name)} <span class="badge">CR ${b.cr || '—'}</span></div>
                    <div class="roster-meta"><span>AC ${b.ac}</span><span>HP ${b.maxHp}</span></div>
                    ${b.notes ? `<div class="hint">${escapeHtml(b.notes)}</div>` : ''}
                </div>
                <div class="icon-actions">
                    <button class="mini on j-spawn" title="Spawn onto the map">➕</button>
                    <button class="mini danger j-rm" title="Remove">🗑️</button>
                </div>`;
            item.querySelector('.j-spawn').addEventListener('click', () => {
                S.update((st2) => {
                    const L = S.activeLocation(st2);
                    L.tokens.push(S.makeToken({
                        name: b.name, kind: 'enemy', color: '#e5484d', hp: b.maxHp, maxHp: b.maxHp, ac: b.ac,
                        c: 1 + (L.tokens.length % Math.max(1, L.map.cols - 2)),
                        r: 1 + (Math.floor(L.tokens.length / 4) % Math.max(1, L.map.rows - 2)),
                        initiative: Math.ceil(Math.random() * 20),
                    }));
                });
                toast(b.name + ' spawned');
            });
    ...
    /* ======================================================================
       BIG BAD
       ====================================================================== */
    $('#bbToMap').addEventListener('click', () => {
        const bb = S.get().bigbad;
        S.update((st) => {
            const L = S.activeLocation(st);
            L.tokens.push(S.makeToken({
                name: bb.name || 'Big Bad', kind: 'enemy', color: '#b91c1c', size: Number(bb.size) || 2,
                hp: bb.hp, maxHp: bb.maxHp, ac: Number(bb.ac) || 15,
                c: Math.floor(L.map.cols / 2), r: Math.floor(L.map.rows / 2), initiative: 20,
            }));
        });
        toast('The Big Bad enters the map');
    });
    ...
    $('#addInvForm').addEventListener('submit', (e) => {
        ...
        S.update((st) => st.inventory.push({ id: 'inv-' + Math.random().toString(36).slice(2, 8), name, qty, note: '' }));
        e.target.reset(); $('#invQty').value = 1; $('#invName').focus();
    });
    function renderInventory() {
        const st = S.get();
        $('#invCount').textContent = st.inventory.length;
        const wrap = $('#invList'); wrap.innerHTML = '';
        st.inventory.forEach((it) => {
            const row = document.createElement('div');
            row.className = 'inv-item';
            row.innerHTML = `
                <span class="inv-name">${escapeHtml(it.name)}${it.note ? `<small>${escapeHtml(it.note)}</small>` : ''}</span>
                <button class="hp-step j-minus">−</button>
                <span class="inv-qty">${it.qty}</span>
                <div style="display:flex;gap:.2rem"><button class="hp-step j-plus">＋</button><button class="mini danger j-del">🗑️</button></div>`;
    ...
    /* ======================================================================
       ...
       ====================================================================== */
    function roll(sides) {
        const qty = clamp(Number($('#diceQty').value) || 1, 1, 20);
        const mod = Number($('#diceMod').value) || 0;
        const adv = $('#diceAdv').checked && sides === 20;
        const rolls = []; let total = 0;
        if (adv) {
            const a = 1 + Math.floor(Math.random() * 20), b = 1 + Math.floor(Math.random() * 20), best = Math.max(a, b);
            rolls.push(`${a}/${b}→${best}`); total = best + mod;
        } else {
            for (let i = 0; i < qty; i++) { const v = 1 + Math.floor(Math.random() * sides); rolls.push(v); total += v; }
            total += mod;
        }
        const box = $('#diceResult'); box.className = 'dice-result';
        ...
        const label = `${adv ? 'adv ' : ''}${qty}d${sides}${mod ? (mod > 0 ? '+' + mod : mod) : ''}`;
        box.querySelector('.detail').textContent = `${label}  [${rolls.join(', ')}]`;
        diceHistory.unshift(`${label} = ${total}  [${rolls.join(', ')}]`);
        diceHistory = diceHistory.slice(0, 8);
        $('#diceLog').innerHTML = diceHistory.map((l) => `<div>🎲 ${l}</div>`).join('');
        if (S.get().settings.showDiceToPlayers) {
            S.update((st) => { st.lastRoll = { label, total, rolls, sides, crit, fumble, t: Date.now() }; });
        }
    }
    ...
    $('#panelCol').addEventListener('click', (e) => {
        const btn = e.target.closest('.pmove'); if (!btn) return;
        e.preventDefault(); e.stopPropagation();
        const panel = btn.closest('[data-panel]'); const id = panel.dataset.panel; const dir = Number(btn.dataset.dir);
        S.update((st) => {
            const ord = st.settings.panelOrder; const i = ord.indexOf(id); const j = i + dir;
            if (i < 0 || j < 0 || j >= ord.length) return;
            ord.splice(i, 1); ord.splice(j, 0, id);
        });
    });
    function applyPanelOrder() {
        const order = S.get().settings.panelOrder || [];
    ...
    (function buildSettings() {
        ...
        SETTINGS.forEach((s) => {
            ...
            let control = '';
            if (s.type === 'toggle') control = `<label class="switch"><input type="checkbox" data-set="${s.key}"><span class="track"></span></label>`;
            else if (s.type === 'select') control = `<select data-set="${s.key}" style="max-width:160px">${s.options.map((o) => `<option value="${o[0]}">${o[1]}</option>`).join('')}</select>`;
            else if (s.type === 'range') control = `<input type="range" data-set="${s.key}" min="${s.min}" max="${s.max}" style="max-width:120px">`;
            row.innerHTML = `<span class="set-label">${s.label}<small>${s.desc}</small></span>${control}`;
        ...
        grid.addEventListener('change', (e) => {
            ...
            S.update((st) => {
                st.settings[key] = val;
                if ((key === 'revealAroundPlayers' && val) || (key === 'lightRadius' && st.settings.revealAroundPlayers)) autoLight(S.activeLocation(st), st.settings);
            });
        });
        grid.addEventListener('input', (e) => {
            const el = e.target.closest('input[type="range"][data-set]'); if (!el) return;
            S.update((st) => { st.settings[el.dataset.set] = Number(el.value); if (st.settings.revealAroundPlayers) autoLight(S.activeLocation(st), st.settings); });
        });
    })();
    function syncSettings() {
        ...
        $$('#settingsGrid [data-set]').forEach((el) => {
            ...
            if (el.type === 'checkbox') el.checked = !!set[k];
            else if (document.activeElement !== el) el.value = set[k];
        });
    }
    ...
    function syncFields() {
        ...
        g('bbName', st.bigbad.name); g('bbCr', st.bigbad.cr); g('bbAc', st.bigbad.ac);
        g('bbHp', st.bigbad.hp); g('bbMaxHp', st.bigbad.maxHp); g('bbTraits', st.bigbad.traits); g('bbNotes', st.bigbad.notes);
        ...
        const ratio = st.bigbad.maxHp > 0 ? clamp(st.bigbad.hp / st.bigbad.maxHp, 0, 1) : 0;
        $('#bbHpBar').style.width = (ratio * 100) + '%'; $('#bbHpBar').style.background = hpColor(ratio);
        ...
        // fog + map toggles (NOT the terrain select — that stays on the DM's chosen paint terrain)
        $('#gridToggle').checked = L.map.grid;
        $('#fogToggle').checked = L.map.fogEnabled;
        ...
        // weather buttons
        $$('.wx').forEach((b) => b.classList.toggle('active', L.weather === b.dataset.wx || (b.dataset.wx === 'none' && (!L.weather || L.weather === 'none'))));
    ...
    $('#exportBtn').addEventListener('click', () => {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
        a.download = 'loregate-' + new Date().toISOString().slice(0, 10) + '.json'; a.click(); URL.revokeObjectURL(a.href);
    });
    $('#importBtn').addEventListener('click', () => $('#importFile').click());
    $('#importFile').addEventListener('change', (e) => {
        const file = e.target.files[0]; if (!file) return;
        const reader = new FileReader();
        reader.onload = () => { try { S.replace(JSON.parse(reader.result)); toast('Campaign loaded'); } catch { toast('That file could not be read'); } };
        reader.readAsText(file); e.target.value = '';
    });
