...
// ---- Galois field math (primitive polynomial 0x11d) ------------------------
...
// ---- RS block tables (versions 1..10) --------------------------------------
// offset order: L=0, M=1, Q=2, H=3. Each entry: [count,total,data, ...].
const RS = {
...
function createData(version, ecl, bytes) {
    ...
    if (buffer.len + 4 <= capacityBits) buffer.put(0, 4); // terminator
...
class Matrix {
    ...
    setupAlignment() {
        ...
        for (const r of pos) for (const c of pos) {
            if (this.m[r][c] !== null) continue; // overlaps finder
    ...
    setupTypeInfo(ecl, mask) {
        ...
        this.m[this.size - 8][8] = true; // dark module
    }
    setupVersionInfo() {
