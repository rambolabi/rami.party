...
window.GLAMOURS = [
    ...
    },
    {
        ...
        what: [
            'Six themes for an app that ships none. Four dark: Midnight (neutral), Obsidian (deep black), Slate (cool blue-grey) and Ember (warm amber). Then Daylight, which keeps the page pale and pushes the colour hard, so priorities and status pills separate at a glance. Then Graphite, a soft greyscale that sits between the two: the glare is gone but the screen is still light. Off by default; Alt+Shift+D flips between your theme and the original.',
        ...
        how: [
            ...
            'The stale highlight reuses the same column matching: it finds the Last Update header, resolves that column’s index once per grid, parses the date out of each row and tags the old ones with an attribute that a generated stylesheet colours. Because the dark themes rotate hue rather than just inverting, red still reads as red once one is on.',
    ...
    },
    {
        ...
        tags: ['connectwise', 'psa', 'service desk', 'tickets', 'triage', 'automation',
               'productivity', 'bulk edit'],
        files: { userscript: 'user-scripts/cw-manage-autopilot.user.js' },
        summary: 'A morning of triage is the same five keystrokes on every ticket. This does them as each ticket opens, and switches itself off on a countdown, so the rule you set at nine cannot still be stamping tickets at five.',
        what: [
            'All incidents: every ticket you open is set to Type “Incident” with the Subtype you picked. The Subtype list is read out of Manage’s own dropdown, so you choose from the values your board actually has rather than typing one and hoping.',
            'All changes: the same, with Type “Change” plus both the Subtype and the Item. Ticking one of the two unticks the other, because a ticket only has one Type.',
            'Priority: pick the priority every ticket should get. It is a menu button in Manage rather than a dropdown, so it is opened and the matching entry is clicked, exactly as you would.',
            'Ticket owner: put your own name in, on every ticket you open. There is a button that copies the name straight out of a Member field if one is on screen, so the spelling matches Manage’s.',
            'Due date: today, tomorrow, or up to two weeks out, with an option never to land on a weekend. The date format is read off the tenant rather than assumed, so a day-first board gets 13/08/2026 and a month-first board gets 08/13/2026.',
            'A countdown at the bottom: you choose 15 minutes to 8 hours, it shows the time left on the panel and on the floating button, and it stops by itself when the clock runs out. That is the whole point: the tool you forgot to switch off is worse than no tool, and this one cannot be forgotten because it is already counting down.',
        ...
        how: [
            ...
            'A ticket is stamped once. Manage pools and re-uses its pod widgets, so “already done” is keyed to a signature of the ticket sitting in the pod (company, board, status, summary), not to the pod element. Open a different ticket and the guard re-arms by itself.',
            'Fields that already have an answer are left alone unless you allow overwriting, and Manage’s own placeholders, “(Unassigned)” and friends, count as empty, because a placeholder is not a decision somebody made.',
            'The option lists are learned, never guessed. A MutationObserver watches for the floating list Manage renders when a dropdown opens and reads the entries out of it: open one yourself and it is remembered, or press “pull in” and it is opened and closed for you. Lists are stored per tenant hostname.',
        ...
        caveats: [
            ...
            'The option lists come from your board. Until a dropdown has been opened once, the list is empty and you can type the value instead; a value Manage would accept if you typed it is a value this will accept too.',
