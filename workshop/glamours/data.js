...
window.GLAMOURS = [
    {
        ...
        what: [
            ...
            'Ticket content — optionally folds the note Manage itself loads on hover into the same card: how many notes came back, who wrote the last one, the note text, and any screenshots pasted into it. The card says it is waiting while Manage fetches, and says so plainly if nothing comes back, so you are never left staring at a gap.',
        ...
        how: [
            ...
            'Images come across as fresh <img> elements with the source copied over — never markup — read from src, from a lazy-loading attribute or from a CSS background, capped at four, sized down, with the spacer and shadow graphics GXT scatters through its tooltips filtered out by size.',
        ...
        caveats: [
            ...
            'The card can only show what is already on screen. A column that is not in your Manage view is not in the card either, an empty cell is left out rather than shown blank, and “ticket content” is whatever Manage’s own note tooltip returns — usually the latest note, not the whole discussion. Its speed is Manage’s speed: the card can start the lookup early and show the answer instantly, but it cannot make the server reply sooner, and if that tooltip is plain text then there are no images in it to show.',
