...
window.RAMI_WORKSHOP = [
    {
        ...
        search: 'A collection of Tampermonkey userscripts and Stylus userstyles that improve and customize third party websites with custom CSS and JavaScript. Includes a ConnectWise Manage PSA comfort pack for eu.myconnectwise.net with four dark mode themes, a hover ticket preview tooltip that shows the full truncated grid row without opening the ticket with a picker for which columns appear such as date entered, entered by, updated by and service board, plus the ticket content with the note count, last note author and note text, highlighting of stale tickets with no update for five days in red amber or violet, automatically setting Subtype and Item to Change when the Type is Change, automatic closing of the Status Note popup on a company, a password manager friendly login with autocomplete fixed, auto fill of company username and SSO email, automatic login through single sign on at auth.connectwise.com, a caps lock warning, and true text only scaling that enlarges fonts without zooming the whole page. Compares the userscript managers Greasemonkey, Tampermonkey, Violentmonkey and FireMonkey and explains the differences, plus how to enable the Allow User Scripts permission or Developer Mode in Chrome and Edge, and every alternative: Stylus userstyles, bookmarklets, DevTools local overrides, uBlock Origin cosmetic filters, Firefox userContent.css and per site browser zoom.',
